...
- When using an FC only array and provisioning RWX block volumes, the "multi_initiator" attribute won't get set properly on the volume. The workaround is to run `group --edit --iscsi_enabled yes` on the Array OS CLI. Be aware of that editing the group will cause a momentary pause of I/O on existing FC data paths (it's less disruptive than a normal controller failover).
- iSCSI does not work and is not supported on routed data networks. The REST API traffic may be routed and is supported.
- The CSP has been tested using iSCSI with up to 1000 `VolumeAttachments` per compute node. HPE recommends not exceeding 500 `VolumeAttachments` per node and leave headroom for emergencies as recovering a failed node will take hours to reschedule. Other limits may apply with other protocols (LUN limits of FC HBAs as an example) and it's always recommended to test the upper bounds before assuming the node resources are enough for both applications and infrastructure resources. A common problem is running out of file descriptors. Increasing the "maxVolumesPerNode" parameter from the default of 250 is explained in the [Helm chart](https://artifacthub.io/packages/helm/hpe-storage/hpe-csi-driver). Regardless of `maxVolumesPerNode` limit being set, do **NOT** exceed more than 4000 paths in total. See [known limitations](../../index.md#known_limitations).
